...
public class ContactOperations {
    ...
    public ContactOperations addAvatar(FriendsClient client, String friendName) {
    ...
    public ContactOperations addProfileAction(String userId) {
        ...
        if (!TextUtils.isEmpty(userId)) {
            mValues.put(KnurekColumns.DATA_PID, userId);
            mValues.put(KnurekColumns.DATA_SUMMARY, mContext.getString(R.string.profile_action));
            mValues.put(KnurekColumns.DATA_DETAIL, mContext.getString(R.string.view_profile));
            mValues.put(Data.MIMETYPE, KnurekColumns.MIME_PROFILE);
            addInsertOp();
        }
        return this;
    }
    ...
    public ContactOperations updateAvatar(FriendsClient client, String friendName, Uri uri) {
        if (!TextUtils.isEmpty(friendName)) {
            byte[] avatarBuffer = client.getAvatar(friendName);
            if (avatarBuffer != null) {
                mValues.clear();
    ...
    private void addUpdateOp(Uri uri) {
        ContentProviderOperation.Builder builder = newUpdateCpo(uri, mIsSyncOperation,
                mIsYieldAllowed).withValues(mValues);
        mIsYieldAllowed = false;
        mBatchOperation.add(builder.build());
    }
